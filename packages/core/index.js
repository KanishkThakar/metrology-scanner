...
export function createClient(baseUrl, fetcher = globalThis.fetch) {
  ...
  return {
    ...
    barcodeIdentity: id => request('/api/inspections/' + encodeURIComponent(id) + '/barcode-identity'),
    compareBarcode: (id, review) => request('/api/inspections/' + encodeURIComponent(id) + '/barcode-identity', json(review)),
