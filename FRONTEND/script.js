...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function resolveActiveBackend() {
    ...
    const probeTargets = window.METROLOGY_API_URL ? [window.METROLOGY_API_URL] : [
      window.location.origin,
      "http://" + host + ":" + port,
      "http://127.0.0.1:" + port,
      "http://localhost:" + port
    ...
    if (window.location.protocol === "https:" && !window.METROLOGY_API_URL) {
  ...
  ocrEngineButtons.forEach(button => button.addEventListener('click', () => {
    if (scanning) return;
  ...
  if (scanButton) {
    scanButton.addEventListener("click", async () => {
      ...
      showElapsed();
      const scanTimer = setInterval(showElapsed, 1000);
